...
template<typename T> class kColVectorView;
...
public:
	...
	// deep copy from VectorView
	kVector& operator=(const kVectorView<T>& rhs) {
		int n = rhs.size();
		myData.resize(n);
		for (int i = 0; i < n; ++i)
		{
	...
	// deep copy from ColVectorView
	kVector& operator=(const kColVectorView<T>& rhs) {
		int n = rhs.size();
		myData.resize(n);
		for (int i = 0; i < n; ++i)
		{
